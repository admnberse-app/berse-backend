...
interface BerseMintonParticipantsProps {
  eventId: string;
  eventTitle: string;
  isOpen: boolean;
  onClose: () => void;
...
export const BerseMintonParticipants: React.FC<BerseMintonParticipantsProps> = ({ 
  eventId, 
  eventTitle, 
  isOpen, 
  onClose 
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      const eventRegistrations = allRegistrations.filter((reg: Registration) => reg.eventId === eventId);
      
      // Simulate some confirmed registrations (in real app, this would come from backend)
      const updatedRegistrations = eventRegistrations.map((reg: Registration, index: number) => ({
        ...reg,
